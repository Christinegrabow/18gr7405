...
%xDoc = xmlread('TCGA-18-5592-01Z-00-DX1.xml');
...
nrow=im_info(1).Height;
ncol=im_info(1).Width;
...
maskfilename = strcat(filename,'_binary_mask.tif');
colormaskfilename = strcat(filename,'_color_mask.tif');
originalfilename = strcat(filename,'_original.tif');
figure;imshow(binary_mask)
saveas(gcf,maskfilename);
figure;imshow(color_mask)
saveas(gcf,colormaskfilename)
p=0;
origImg=imread(im_file);
figure;imshow(origImg,[])
saveas(gcf,originalfilename)
