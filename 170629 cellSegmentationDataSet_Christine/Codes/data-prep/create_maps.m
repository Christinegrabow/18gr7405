...
fileId = fopen('/Volumes/Surabhi/temp.txt');
num=xlsread('/Volumes/Surabhi/BTP/BTP_files/annotations/xy_details.xlsx');

for z=1:23
    tline = fgetl(fileId);
    xml=strcat(tline(1:28),'.xml');
    xml_file=strcat('/Volumes/Surabhi/BTP/BTP_files/annotations/',xml);
    origin_x=num(z,1);
    origin_y=num(z,2);
    % origin_x = 10144; 78
    % origin_y = 19261;
    ...
    % img(:,:,2)=final;
    % img(:,:,3)=final;
    imwrite(final,strcat('/Volumes/Surabhi/BTP/BTP_files/annotations/z/', strcat(tline(1:32),'.png')));
end
